...
- (void)viewDidLoad {
    ...
    NSLog(@"array image %@",[self getImagesFromURL:linkArray]);
}
...
-(void)addImageToScrollView:(NSArray *)imageViewArray toScrollView:(UIScrollView *)anyScrollView{
    ...
    for (int index = 0; index < imageViewArray.count; index++) {
        ...
        labelPage.text = [NSString stringWithFormat:@"%d / %d",index + 1,imageViewArray.count];
        labelPage.textColor = [UIColor whiteColor];
        labelPage.font = [UIFont systemFontOfSize:12];
        CGFloat width = [self measureTextWidth:[NSString stringWithFormat:@"%d / %d",index + 1,imageViewArray.count] constrainedToSize:CGSizeMake(2000.0f, 12) fontSize:12];
